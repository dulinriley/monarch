...
from monarch.job.kubernetes import ImageSpec, KubernetesJob, KubeConfig
...
def main():
    ...
    parser.add_argument("--out-of-cluster", action="store_true", help="Use kubeconfig for out-of-cluster access")
    args = parser.parse_args()

    job = KubernetesJob(
        namespace="monarch-tests", kubeconfig=KubeConfig.from_path("~/.kube/config") if args.out_of_cluster else None
    )
    if args.volcano:
