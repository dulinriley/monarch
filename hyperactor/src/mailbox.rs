...
impl Mailbox {
    ...
    pub fn open_reduce_port<A, S>(
        &self,
        accum: A,
    ) -> (OncePortHandle<A::State>, OncePortReceiver<A::State>)
    where
        A: Accumulator<State = S, Update = S> + Send + Sync + 'static,
        S: Message + Default + Clone,
    {
        let port_index = self.inner.allocate_port();
        let (sender, receiver) = oneshot::channel::<A::State>();
        let port_id = PortId(self.inner.actor_id.clone(), port_index);
        let reducer_spec = accum.reducer_spec();
        assert!(
...
>>>>>>> source: e0ea38172b3a - dulinr: [monarch] Introduce Mailbox::open_accu...
    }
}
