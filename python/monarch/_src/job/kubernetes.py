    ...
    *Provisioning* -- create MonarchMesh CRDs via the K8s API so the
        ...
        try:
            for event in w.stream(
                ...
                if self._is_pod_worker_ready(pod):
                    ...
                    logger.info("Pod '%s' is ready with rank %d: %s", pod.metadata.name, pod_rank, ready_pods_by_rank[pod_rank])
    ...
    def _setup_out_of_cluster(self, pod_endpoints: Iterable[_MonarchMeshPod]) -> None:
        ...
        for pod in pod_endpoints:
            ...
            try:
                cmd = ["kubectl", "port-forward", "--namespace", self._namespace, f"pod/{pod.name}", f":{duplex_port}"]
                ...
                duplex_addr = f"tcp://127.0.0.1:{local_port}"
            except Exception as e:
                raise RuntimeError(f"Failed to set up port forwarding for pod {pod.name} with kubectl") from e

        if duplex_addr:
            # Found at least one duplex port, use it as the location for "context".
            # This must be the first call to context or it'll use the wrong client mailbox.
            logger.info("Overwriting context to: %s", duplex_addr)
            context(attach_to=duplex_addr)
        # If no duplex address is found, that's ok. The user may be spawning
        # multiple meshes and we only need one of them to have a duplex address
        # exposed.
        # If none of them have an address exposed, it'll be a connection error
        # to the frontend address.
    ...
    def _state(self) -> JobState:
        ...
        for mesh_name, mesh_config in self._meshes.items():
            ...
            logger.info("Calling attach_to_workers(name=%s, workers=%s)", mesh_name, workers)
